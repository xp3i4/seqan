...
int main(int argc, char** argv)
{
    ...
    int minLength = atof(argv[3]);
    int kmerLength = atof(argv[5]);
    ...
    //    count = 0;
        ...
        {  
            countWindow[position(pattern).i2 / windowLength]++;
            count++;
        }
        for (unsigned j = 0; j < 1500; j++ )
        { 
    ...
    std::cout <<  minLength << " " << errorRate << " " << kmerLength << " " << sysTime() - start << std::endl;
    //print(frq);
